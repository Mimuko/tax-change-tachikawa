...
練馬区×介護は **MY-156 で移植性監査済み**（公開・データ追加は未着手）。立川市×教育は MY-155 で公開済み（下記）。介護の職員/賃金表示は既存の可用性判定を引き継いでいる。掲載しない指標の共通ルールは `docs/data-definition.md` §掲載しない指標（DataGap）。

## MY-156: 練馬区×介護 移植性監査（2026-09-17）

`/nerima/care` は **未公開**（`story-registry` 未登録）。監査の正本:
