...
`/[municipality]/[topic]/` とその `data/` を登録一覧から静的生成する。未知の組み合わせは404。`/` は machinohenka の TOP（コンセプト説明と公開ストーリー一覧）、`/data/` はデータ方針ハブ。詳細ページのナビは `src/components/site-topbar.tsx` が `story-registry` から兄弟ストーリーへリンクする。一覧の集約は `src/lib/site-hub.ts`。
